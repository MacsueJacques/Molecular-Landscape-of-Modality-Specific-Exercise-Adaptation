...
res_df_down <- res_df %>% filter(grepl("Under", diffexpressed)) %>% 
  dplyr::select(!c('minuslog10padj', 'diffexpressed')) 
rownames(res_df_down) <- res_df_down$ID

# For visualisation purposes, let's shorten the pathway names
res_df_down$Description <- gsub('(H|h)iv', 'HIV', 
                              gsub('pd 1', 'PD-1',
                                   gsub('ecm', 'ECM', 
                                        gsub('(I|i)nterleukin', 'IL', 
                                             gsub('(R|r)na', 'RNA', 
                                                                                          ...
                                                                                          gsub('mhc', 'MHC',
                                                                                               gsub('mhc class i', 'MHC I', 
                                                                                                    gsub('mhc class ii', 'MHC II', 
                                                                                                         stringr::str_to_sentence(
                                                                                                           gsub('_', ' ',  
                                                                                                                gsub('REACTOME_', '', res_df_down$Description)))))))))))))))))))
...
res_df_down <- res_df %>% filter(grepl("DOWN", diffexpressed)) %>% 
  dplyr::select(!c('minuslog10padj', 'diffexpressed')) 
rownames(res_df_down) <- res_df_up$ID
...
tiff('dot plot Over Proteins.tiff',
     ...
     units = 'mm',
     res=300)
dotplot(enrichres_up, showCategory = 6) + ggtitle("Top pathways from over expressed Proteins")
dev.off()
